...
## fpssil primate reanalysis with DEC+J 
setwd("Biogeography/fossil_only/")
trfn = "/BioGeoBEARS/PrimatesFossilBiogeog_nozero.tre"
tr <- read.tree(trfn)
geogfn = "fossilTaxa.phy"
...
write.table(range_probabilities[-(1:length(tr$tip.label)),]
            , file="DEC_range_probabilities_biogeobears.txt", quote=FALSE, sep="\t")

tr$node.label <- unique(tr$edge[,1])
plot(tr, show.tip.label = F)
nodelabels(cex=0.4)
write.tree(tr, "DEC_range_labels_biogeobears.tre")
tr$node.label<-colnames(range_probabilities)[apply(range_probabilities, 1, which.max)]
write.tree(tr, "DEC_range_biogeobears.tre")
...
# Write the table to a tab-delimited text file (for Excel etc.)
write.table(range_probabilities[-(1:length(tr$tip.label)),]
            , file="DECJ_range_probabilities_biogeobears.txt", quote=FALSE, sep="\t")

tr$node.label <- unique(tr$edge[,1])
plot(tr, show.tip.label = F)
nodelabels(cex=0.4)
write.tree(tr, "DECJ_range_labels_biogeobears.tre")
tr$node.label<-colnames(range_probabilities)[apply(range_probabilities, 1, which.max)]
write.tree(tr, "DECJ_range_biogeobears.tre")
